...
for i1 = 1:length(datasetCandi)
    ...
    if ~exist(fname2, 'file')
        ...
        for iParam = 1:nParam
            ...
            for iRepeat = 1:nRepeat
                ...
                if exist(fname3, 'file')
                    ...
                    [label, Yc, Z, Gs, objHistory, evalHistory] = TMOE_MVKM_v2(Xs, nCluster, nGroup);
                    t = toc(t1_s);
                    res_i = my_eval_y_2025_m7(label, Y)';
                    TMOE_MVKM_v2_res(iParam, iRepeat, :) = res_i;
                    TMOE_MVKM_v2_res_his{iParam, iRepeat} = cell2mat(evalHistory);
                    save(fname3, 'res_i', 't');
                end
                seedplus = seedplus + 1;
        ...
        save(fname2, 'TMOE_MVKM_v2_result_summary','TMOE_MVKM_v2_std_summary', 'TMOE_MVKM_v2_res', 'TMOE_MVKM_v2_time','TMOE_MVKM_v2_res_his','TMOE_MVKM_v2_res_good_idx');
    end
